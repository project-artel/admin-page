...
10. Open a draft PR with evidence and explicit remaining risk, as soon as the
    work is ready and without waiting to be asked. Follow `pull-request.md` for
    draft ownership, user notification, assignee, and type-label rules.
...
7. **Open the draft PR.** Do this as soon as the work is ready, without waiting to be
...
1. **Create the umbrella issue.** Issue type `작업`, 레포지토리 `없음`, parented
   ...
   The umbrella is the one issue that does not carry 레포지토리 `admin-page`;
   the `admin-page`-only rule in step 1 above governs this repository's own child
   issue, which still takes `ARTEL-248` as its `parent`.
...
3. **Run steps 2–7 of `## Jira-Driven Development Flow` once per repository
