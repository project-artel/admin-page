import { NODE_RADIUS, type PositionedNode } from './layout'
...
export type LabelOptions = {
  ...
  /** Incident relation count, which decides who keeps a name when space runs out. */
...
export function placeLabels(
  ...
  label: (node: PositionedNode) => string,
  options: LabelOptions,
): Map<string, LabelPlacement> {
  const placements = new Map<string, LabelPlacement>()
  const taken: Box[] = []

  for (const placed of [...nodes].sort(byPriority(options.keep, options.degree))) {
 ...
 */
function byPriority(keep: ReadonlySet<string>, degree: (id: string) => number) {
  return (left: PositionedNode, right: PositionedNode): number => {
    const leftKept = keep.has(left.node.id)
    const rightKept = keep.has(right.node.id)
    if (leftKept !== rightKept) return leftKept ? -1 : 1
    const leftDegree = degree(left.node.id)
    const rightDegree = degree(right.node.id)
